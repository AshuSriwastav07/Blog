...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Show user details on the UI with a placeholder and error handling
        val profileImageUrl = FirebaseAuth.getInstance().currentUser?.photoUrl
        Picasso.get()
            .load(profileImageUrl)
            .placeholder(R.drawable.baseline_person_24) // Placeholder image while loading
            .error(R.drawable.baseline_person_24) // Error image if the load fails
            .into(binding.userProfile, object : Callback {
